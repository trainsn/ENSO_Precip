...
def backward_prop():
    ...
    input_feat_min = np.array([95827.602, 199.63177, 4644.373, 4644.373, -21.138626, -21.138626, 267.872]).reshape((1, -1, 1, 1, 1))
    input_feat_max = np.array([104431.23, 314.57242, 5970.5391, 5970.5391, 87.681274, 87.681274, 309.222]).reshape((1, -1, 1, 1, 1))
    input_feat_min = torch.from_numpy(input_feat_min).cuda()
    input_feat_max = torch.from_numpy(input_feat_max).cuda()
    grad = grad[0]
    grad = grad * ((input_feat_max - input_feat_min) / 2.)
    for j in range(grad.shape[2]):
        print("\t{:d}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}, {:.4f}".
              format(j, abs(grad[0, 0, j]).sum().item(), abs(grad[0, 1, j]).sum().item(), abs(grad[0, 2, j]).sum().item(),
                     abs(grad[0, 3, j]).sum().item(), abs(grad[0, 4, j]).sum().item(), abs(grad[0, 5, j]).sum().item(),
                     abs(grad[0, 6, j]).sum().item()))
    grad_stats = abs(grad).sum((3, 4))
...
def retrieve_variable_time():
    ...
    retrieved_sensitivity = grad[0, variable_idx, relative_month].detach().cpu().numpy()
