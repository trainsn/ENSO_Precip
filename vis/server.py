...
def forward_prop():
    ...
    output_precip = fake_precip[0, 0, 0].clone()
    output_precip[precip_mask] = precip[precip_mask]
    print(time_idx, (abs(output_precip - precip).sum() / (~precip_mask).sum()).item(), abs(output_precip - precip).max().item())

    output_precip = output_precip[int(44.5 * multiplier + 0.5):int(69.5 * multiplier + 0.5),
                  int(80 * multiplier + 0.5):int(138.5 * multiplier + 0.5)].unsqueeze(0).unsqueeze(0)
    output_precip = F.interpolate(output_precip, scale_factor = 2., mode="bilinear")
    output_precip = output_precip[0, 0].detach().cpu().numpy()
    ...
    output_image = colormap.cmap(output_precip, viridis, -1., -0.25)
...
def backward_prop():
    ...
    time_idx = int(request.form["time_idx"]) + recep_field - 1
